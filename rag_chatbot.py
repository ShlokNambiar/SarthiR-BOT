...
from pipeline.query_interface import initialize_pinecone, get_query_embedding, search_pinecone
...
# Check for web search environment variable - handle various true values
web_search_env = os.getenv("ENABLE_WEB_SEARCH", "false").lower()
WEB_SEARCH_ENABLED = web_search_env in ["true", "1", "yes", "y", "on", "enabled"]  # Enable web search
print(f"Web search enabled from environment: {WEB_SEARCH_ENABLED} (value: {web_search_env})")
WEB_SEARCH_THRESHOLD = 0.85  # Minimum relevance score threshold for RAG results (higher = more likely to use web search)
...
def create_chat_prompt(
    ...
    system_message = {
        ...
        "content": (
            ...
            "Regulations (UDCPR) for Maharashtra State. Your primary task is to provide accurate, "
            "helpful information based on the UDCPR document in a conversational and engaging manner. "
            "When answering UDCPR-related questions, use the context provided, but present the information in a "
            "natural, conversational way rather than directly quoting the document. "

            "IMPORTANT: You can also answer general knowledge questions that are not related to UDCPR. "
            "For these questions, you MUST use the web search results provided to give accurate and up-to-date "
            "information. This includes questions about weather, current events, locations, or any other topic "
            "outside of UDCPR regulations."

            "INSTRUCTIONS FOR HANDLING WEB SEARCH RESULTS: "
            "1. For ANY question where web search results are provided, you MUST use this information in your answer. "
            "2. For questions about weather, temperature, current events, or real-time information, the web search "
            "   results are your primary source of information - use them extensively. "
            "3. DO NOT mention sources or cite websites in your response. "
            "4. Present the information as factual knowledge, not as something you found from a search. "
            "5. For questions specifically asking about 'most recent' or 'latest' information, prioritize the "
            "   web search results even if you think you have some information in your knowledge base."
            "6. If the web search results contain real-time information like current weather, prices, or events, "
            "   include this specific information in your answer rather than giving a generic response."

            "If the web search results don't provide clear or specific information about the question, "
            "acknowledge this limitation but still provide what you know."
            ...
            "When providing information from the UDCPR document, focus on the content itself rather than citations. "
